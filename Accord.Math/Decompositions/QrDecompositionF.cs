...
namespace Accord.Math.Decompositions
{
    ...
    public sealed class QrDecompositionF : ICloneable, ISolverMatrixDecomposition<float>
    {
        ...
        public QrDecompositionF(float[,] value)
            : this(value, false)
        {
        }
        ...
        public QrDecompositionF(float[,] value, bool transpose)
        {
            if (value == null)
            {
            ...
            this.qr = transpose ? value.Transpose() : (float[,])value.Clone();

            int rows = qr.GetLength(0);
            int cols = qr.GetLength(1);
            this.Rdiag = new float[cols];
